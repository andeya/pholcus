...
const (
	...
	FULL_NAME string = NAME + "_" + VERSION + " （by " + AUTHOR + "）" // Full name
...
const (
	WORK_ROOT      string = TAG + "_pkg"                    // Runtime directory name
	CONFIG         string = WORK_ROOT + "/config.ini"       // Config file path
	CACHE_DIR      string = WORK_ROOT + "/cache"            // Cache directory
	LOG            string = WORK_ROOT + "/logs/pholcus.log" // Log file path
	LOG_ASYNC      bool   = true                            // Whether to output logs asynchronously
	PHANTOMJS_TEMP string = CACHE_DIR                       // Surfer-Phantom: temp dir for JS files
	HISTORY_TAG    string = "history"                       // History record identifier
	HISTORY_DIR    string = WORK_ROOT + "/" + HISTORY_TAG   // History dir for excel/csv output
	SPIDER_EXT     string = ".pholcus.xml"                  // Dynamic rule extension (recommended)
	SPIDER_EXT_OLD string = ".pholcus.html"                 // Dynamic rule extension (legacy)
)
...
var (
	...
	BeanstalkdTube           string = setting.DefaultString("beanstalkd::tube", beanstalkTube)             // Beanstalkd tube name
...
func init() {
	cache.Task = &cache.AppConf{
		...
		SuccessInherit: setting.DefaultBool("run::success", success),          // Inherit success history
		FailureInherit: setting.DefaultBool("run::failure", failure),          // Inherit failure history
	}
}
